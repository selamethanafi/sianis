...
if($permanen == 0)
{
	...
	if($adatb>0)
	{
		...
		$adatc = $tc->num_rows();
		echo form_open('kepala/revisiskp/'.$tahun.'/'.$nip,'class="form-horizontal" role="form"');
		...
		foreach($tb->result() as $b)
			...
			if($adatc == 0)
			{
				$this->db->query("insert into `skp_skor_guru_revisi` (`tahun`, `nip`, `id_skp_skor_guru_revisi`) values ('$tahun', '$nip', '$id')");
			}	
			...
			echo '<div class="form-group row"><div class="col-sm-3"><label class="control-label">Angka kredit</label></div><div class="col-sm-9"><p class="form-control">'.$ak.'</p>';
echo 'Golongan '.$gol1.' 100% = '.$p100.' 75% = '.$p75.' 50% = '.$p50.' 25% = '.$p25.' 5% = '.$p5.' 2% = '.$p2; echo '</div></div>';
			echo '<div class="form-group row"><div class="col-sm-3"><label class="control-label">Realisasi Angka kredit</label></div><div class="col-sm-9"><input type="text" name="ak_r" value="'.$rak_r.'" class="form-control"></div></div>';
