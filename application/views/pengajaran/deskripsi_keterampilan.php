...
if(($kurikulum == '2013') or ($kurikulum == '2015'))
{
	if($id <= $total_siswa)
	{
			...
			do
			{
				...
				if (!empty($p[$item]))
				{
					...
					else
					{
						...
						{
							$ketpsi .= ' <p class="text-danger">deskripsi penilaian ke-'.$item.' tidak bisa dibuat karena tidak ada nilai.</p>';
						}
		...
		<script>setTimeout(function () {
		   window.location.href= '<?php echo base_url();?>pengajaran/deskripsiketerampilan/<?php echo $id_mapel?>/<?php echo $id?>';
			},1);
			</script>
		<?php
	}
	else
	{
		...
		<script>setTimeout(function () {
		   window.location.href= '<?php echo base_url();?>pengajaran/periksanilairapor/<?php echo $tahun1;?>/<?php echo $semester;?>/<?php echo $kodeguru;?>';
		},100);
			</script>
		<?php
	}
