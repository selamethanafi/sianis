...
<?php
		...
		$noitem = 1;
		$totalkekurangan = 0;
$ta = $this->db->query("select DISTINCT `thnajaran`, `kelas` FROM `siswa_kelas` where `nis`='$nis' order by thnajaran ASC");
foreach($ta->result() as $a)
{
		...
		foreach($tabel_pembayaran->result() as $c)
		{
			...
			if($kurang>0)
			{
				...
				elseif(($besar_ajuan_sekarang <= $kurang) and ($besar_ajuan_sekarang > 0))
				{
					...
					<div class="col-sm-6"><input name="keterangan_'.$noitem.'" type="text" placeholder="keterangan tambahan" class="form-control"></div></div>';
					$terproses = $terproses + $besar_ajuan_sekarang;
					$besar_ajuan_sekarang = 0;
...
$cacah_item2 = 0;
$noitem2 = 1;
if($besar_ajuan_sekarang > 0)
{
	echo '<div class="alert alert-danger"><h2>Ada sisa '.xduit($besar_ajuan_sekarang).'</div>';
// Terproses '.$terproses;
}
if($besar_ajuan_sekarang>0)
{
