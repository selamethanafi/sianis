...
{
	?>
	<p><a href="<?php echo base_url().'keuangan/massal/'.$tahun1.'/'.$semester;?>" class="btn btn-info">Kelas Lain</a></p>
	...
	<tr align="center"><td width="30"><strong>No.</strong></td><td><strong>NIS</strong></td><td><strong>Nama</strong></td><td><strong>Tanggal</strong></td><td><strong>Macam</strong></td><td><strong>Jumlah</strong></td><td><strong>Hapus</strong></td></tr>
		...
		foreach($querysiswa->result() as $c)
		{
			...
			foreach($tb->result() as $b)
			{
				...
				echo "<td align=\"center\"><a href='".base_url()."keuangan/massal/".$tahun1."/".$semester."/".$id_walikelas."/".$id_macam_pembayaran."/sukses/hapus/".$b->id_siswa_bayar."' onClick=\"return confirm('Anda yakin ingin menghapus record ini?')\" title='Hapus Pembayaran Siswa'><span class=\"fa fa-trash-alt\"></span></a></td></tr>";
			$nomor++;
			}
		}
		echo '</table></div>';
...
{
$xloc = base_url().'keuangan/massal';
if((!empty($tahun1)) and (!empty($semester)) and (!empty($id_walikelas)) and (!empty($id_macam_pembayaran)))
{
	echo '<form name="formx" method="post" action="'.$xloc.'/'.$tahun1.'/'.$semester.'/'.$id_walikelas.'/'.$id_macam_pembayaran.'" class="form-horizontal" role="form">';
}
else
{
...
if(!empty($id_macam_pembayaran))
{
	$tc = $this->db->query("select * from `m_uang_besar` where `id_uang_besar`='$id_macam_pembayaran'");
	$macam_pembayarane = '';
	foreach($tc->result() as $c)
	{
		$macam_pembayarane = $c->macam_pembayaran;
	}
	?>
	<div class="form-group row"><div class="col-sm-3"><label class="control-label"><a href="<?php echo $xloc.'/'.$tahun1.'/'.$semester.'/'.$id_walikelas;?>">Macam Pembayaran</a></label></div><div class="col-sm-9"><p class="form-control-static"><input name="id_macam_pembayaran" type="hidden" value="<?php echo $id_macam_pembayaran;?>" class="form-control"><input type="text" value="<?php echo $macam_pembayarane;?>" class="form-control" readonly></p></div></div>
	<?php
}
...
elseif(empty($id_macam_pembayaran))
{
	$tingkat = kelas_jadi_tingkat($kelas);
	$tabel_pembayaran=$this->Keuangan_model->Daftar_Besar_Pembayaran($tingkat,$thnajaran);	
	?>
	<div class="form-group row"><div class="col-sm-3"><label class="control-label">Macam Pembayaran</label></div><div class="col-sm-9">
	<select name="id_macam_pembayaran"  onChange="MM_jumpMenu('self',this,0)" class="form-control">
	<?php
	echo '<option value=""></option>';

	foreach($tabel_pembayaran->result() as $b)
	{
	echo '<option value="'.$xloc.'/'.$tahun1.'/'.$semester.'/'.$id_walikelas.'/'.$b->id_uang_besar.'">'.$b->macam_pembayaran.'</option>';
	}
	?>
	</select></div></div>
	<?php
...
else
{
	...
	?>
	<div class="table-responsive"><table class="table table-striped table-hover table-bordered">
	<tr align="center"><td width="30"><strong>No.</strong></td><td><strong>NIS</strong></td><td><strong>Nama</strong></td><td><strong>Kekurangan</strong></td><td><strong>Jumlah Bayar</strong></td><td><strong>Keterangan</strong></td></tr>
		<?php
		$querysiswa=$this->Admin_model->Tampil_Siswa_Kelas($thnajaran,$semester,$kelas);
		$nomor=1;
		foreach($querysiswa->result() as $c)
		{
			...
			$totalkekurangan = 0;
			$td = $this->db->query("SELECT * FROM `m_uang_besar` where `id_uang_besar`='$id_macam_pembayaran'");
			$tagihan = 0;
			foreach($td->result() as $d)
			{
				$tagihan = $d->besar;
			}
			$tc = $this->db->query("select * from siswa_bayar where macam_pembayaran='$macam_pembayarane' and thnajaran='$thnajaran' and nis='$nis'");
			$terbayar = 0;
			foreach($tc->result() as $c)
			{
			...
			if($kurang>0)
			{
				...
				?>
				<td align="right"><input type="hidden" name="macam_pembayaran" value="<?php echo $macam_pembayarane;?>"><?php echo number_format($kurang);?> </td>
		...
		echo '</table></div>';		$cacah = $nomor - 1;
		echo '<input type="hidden" name="cacah" value="'.$cacah.'"><input type="hidden" name="thnajaran" value="'.$thnajaran.'">';	
	echo '<p class="text-center"><input type="submit" value="Simpan Data" class="btn btn-primary">&nbsp;&nbsp;&nbsp;<a href="'.base_url().'keuangan/massal" class="btn btn-info"><b>Batal</b></a></p>';
