...
$namamapel = '';
			...
			if($adanilai>0)
			{
				foreach($tc->result() as $c)
				{
					...
					elseif($status_nilai == 'sementara')
					{
						...
						echo '<td align="center">'.predikat_nilai_2015($c->nilai_nr,$this->config->item('versi_predikat_nilai')).'</td>';
						echo '<td align="center">'.$c->psikomotor.'</td>';
						echo '<td align="center">'.predikat_nilai_2015($c->psikomotor,$this->config->item('versi_predikat_nilai')).'</td>';
					}
					else
					{
	...
	echo '<table width="100%">
		<tr><td width="30%">Mengetahui</td><td width="36%"></td><td>'.$this->config->item('lokasi').', '.date_to_long_string($tanggalcetak).'</td></tr>
