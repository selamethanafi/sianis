...
<?php
$id = $id - 1;
$ta = $this->db->query("select * from `aspek_psikomotorik_old` order by `id_aspek_psikomotor`");
$adata = $ta->num_rows();
$ta = $this->db->query("select * from `aspek_psikomotorik_old` order by `id_aspek_psikomotor` limit $id,1");
foreach($ta->result() as $a)
{
	...
	$np= hilangkanpetik($a->np);
	$this->db->query("update `m_mapel` set `p1` = '$p1', `p2` = '$p2', `p3` = '$p3', `p4` = '$p4', `p5` = '$p5', `p6` = '$p6', `p7` = '$p7', `p8` = '$p8', `p9` = '$p9', `p10` = '$p10' , `p11` = '$p11' , `p12` = '$p12' , `p13` = '$p13' , `p14` = '$p14' , `p15` = '$p15', `p16` = '$p16', `p17`='$p17', `p18`='$p18', `np`='$np' where `thnajaran` = '$thnajaran' and `semester`='$semester' and `mapel`='$mapel' and `kelas`='$kelas'");
}
$id2 = $id + 1;
if(($id <= $adata) and ($adata > 0))
{
	...
	<script>setTimeout(function () {
		   window.location.href= '<?php echo base_url();?>admin/pindahaspekpsikomotor/<?php echo $id2;?>';
	},1);
			</script>
	<?php
}
