...
if($permanen == 0)
{
	if(($status=='baru') or ($status=='lama'))
	{
		...
		{
			$kegiatan .= '<h2><strong>Golongan '.$golongan.' tidak sesuai dengan aplikasi, mutakhirkan data SK Kepegawaian per semester <a href="'.base_url().'pkg/sk">di sini</a> lalu mutakhirkan data skp unsur tugas relevan.</strong></h2><BR>';
		}
		$tz = 	$this->db->query("select * from `skp_skor_guru` where `kode` = '00' and `nip` = '$nippegawai' and `tahun`='$tahunpenilaian'");
		$ada = $tz->num_rows();
		if($ada == 0)
		{
			$this->db->query("INSERT INTO `skp_skor_guru` (`kode`,`unsur`, `kegiatan`, `ak`,`ak_target`,`kuantitas`, `satuan`, `kualitas`, `waktu`, `satuanwaktu`, `biaya`, `nip`, `tahun`,`status`) VALUES ('00', 'B', '$kegiatan', '$ak', '$ak_target','1', 'Laporan', '100', '$waktu', 'bl', '0', '$nippegawai', '$tahunsekarang','0')");
		} // akhir kalau baru
		else
		{
			$this->db->query("update `skp_skor_guru` set `kegiatan`='$kegiatan', `ak`='$ak', `ak_target`='$ak_target', `status`='0', `waktu`='$waktu' where `nip`='$nippegawai' and `tahun`='$tahunsekarang' and `kode`='00'");
		}
		...
		if($gurubk == 0)
		{
			...
			{
				$ta=$this->db->query("select * from `skp_skor_guru` where `tahun`='$tahunsekarang' and `nip`='$nippegawai' and `kode`='01'");
				$ada = $ta->num_rows();
				if($ada == 0)
				{
					$this->db->query("INSERT INTO `skp_skor_guru` (`kode`,`unsur`, `kegiatan`, `ak`,`ak_target`,`kuantitas`, `satuan`, `kualitas`, `waktu`, `satuanwaktu`, `biaya`, `nip`, `tahun`,`status`) VALUES ('01', 'B', '$kegiatan1', '$ak', '$aktambahan_target','1', 'Laporan', '100', '$waktu', 'bl', '0', '$nippegawai', '$tahunsekarang','0')");
				}
				else
				{
					$this->db->query("update `skp_skor_guru` set `kegiatan`='$kegiatan1', `ak`='$ak', `ak_target`='$aktambahan_target', `status`='0' , `waktu`='$waktu' where `nip`='$nippegawai' and `tahun`='$tahunsekarang' and `kode`='01'");
				}
			}
		}
		if((empty($tambahan)) or ($adatambahan == 0))
		{
			$this->db->query("delete from `skp_skor_guru` where `nip`='$nippegawai' and `tahun`='$tahunsekarang' and `kode`='01'");
		}
		header('Location: '.base_url().'pkg/skp/'.$status.''); //redirect browser to public main page
	}
	$ta=$this->db->query("select * from `skp_skor_guru` where `tahun`='$tahunsekarang' and `nip`='$nippegawai' and `unsur`='B' and `kegiatan` like 'Unsur%'");
	...
	if($bisa == 1)
	{
		echo form_open('pkg/tambahskp/pkg','class="form-horizontal" role="form"');
		echo '
		...
		if (($golongan=='III/a') or ($golongan=='III/b') or ($golongan=='III/c') or ($golongan=='III/d') or ($golongan=='IV/a') or ($golongan=='IV/b') or ($golongan=='IV/c') or ($golongan=='IV/d'))
		{
			echo '<input type="text" name="golongan" class="form-control" value="'.$golongan.'" readonly></div></div>
		...
		{
			echo '<h4>Golongan <strong>'.$golongan.'</strong> tidak sesuai dengan aplikasi, mutakhirkan data SK Kepegawaian per semester <a href="'.base_url().'pkg/sk" class="btn btn-info">di sini</a> lalu mutakhirkan data skp unsur tugas relevan.</h4>';
		}
		echo '</form>';
	}
}
