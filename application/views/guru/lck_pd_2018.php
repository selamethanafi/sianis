...
foreach($ta->result() as $a)
	...
	echo '<h3><p class="text-center">RAPOR  Versi 2018</p></h3>';?>
...
<tr><td align="center">Angka</td><td align="center">Predikat</td><td align="center">Angka</td><td align="center">Predikat</td></tr>';
	...
	foreach($tc->result() as $c)
	{
		...
		else
		{
			...
			{
				$td = $this->db->query("select * from `nilai` where `mapel` like '%prakarya%' and `thnajaran`='$thnajaran' and `kelas`='$kelas' and `semester`='$semester' and `nis`='$nis' and `status`='Y'");
				foreach($td->result() as $d)
				{
					$mapelportal = $d->mapel;
				}
			...
			if(empty($namaguru))
			{
				$namaguru = '<div class="alert alert-danger">Siswa tidak punya nilai</div>';
			}
			//cari nilai
			...
			foreach($td->result() as $d)
			{
				...
				$qb = predikat_nilai_2018($nilai_pengetahuan,$kkm);
				$qd = predikat_nilai_2018($nilai_keterampilan,$kkm);
				if (substr($ket,0,5)=='Belum')
				{
					$ket = '<font color="#FF0000">'.$ket.'</font>';
				}
				...
				if (empty($kunci))
				{
					echo '<a href="'.base_url().'guru/detilsiswa/'.$nis.'/'.$id_walikelas.'/'.$item.'/kunci/'.$kd.'"><span class="fa fa-lock"></span>';
					$cacahkunci++;
				}
				else
				{
...
//deskripsi capaian
			...
			foreach($th->result() as $h)
				{
					...
					if($mapel == 'Prakarya dan Kewirausahaan')
					{
						$td = $this->db->query("select * from `$tabel` where `mapel` like '%prakarya%' and `thnajaran`='$thnajaran' and `kelas`='$kelas' and `semester`='$semester' and `nis`='$nis' and `status`='Y'");
					}
					else
					{
					...
					if($mengikuti>0)
						...
						{
						echo '<tr><td width="5%" align="center">'.$komponen.'</td><td width="25%">'.$mapel.'</td><td colspan="2"><div class="alert alert-danger">Siswa tidak punya nilai</div></td></tr>';
						$nomor++;
						}		
	...
	?>
	<p class="text-center"><a href="javascript:;" onClick="window.open('<?php echo base_url();?>pdf_report/bukulck/<?php echo substr($thnajaran,0,4).'/'.$semester.'/'.$nis.'/akhir/2018/'.$ttd;?>','yes','scrollbars=yes,width=550,height=400')" title="pastikan nilai sudah terkunci semua" class="btn btn-success"><strong>LHB PDF</strong></a>  <a href="javascript:;" onClick="window.open('<?php echo base_url();?>guru/rapor/<?php echo substr($thnajaran,0,4).'/'.$semester.'/'.$nis.'/akhir/'.$kurikulum;?>','yes','scrollbars=yes,width=550,height=400')" class="btn btn-primary" title="cocok kalau menggunakan google chrome atau chromium"><span class="fa fa-print"> <strong>LHB HTML</strong> </a> 
	<?php
	}
	else
	{
