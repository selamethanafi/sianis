...
$trph = $this->db->query("select * from `guru_rph_ringkas` where `lab` like '%$lab%' and thnajaran='$thnajaran' and semester='$semester' order by tanggal ASC, jamke ASC");
...
$nomor =1;
	...
	foreach($trpp2->result() as $rpp2)
		{
		$materi_selanjutnya = $rpp2->materi_pembelajaran;
		}
	...
	$materi = strip_tags($materi);
	$materi = substr($materi,0,250);
	if (substr($keterangan,0,5) != 'TIDAK')
		{
		echo '<tr valign="top"><td align="center" width="30">'.$nomor.'</td><td width="100">'.$dinane.', '.date_to_long_string($d->tanggal_bph).'</td><td align="center" width="30">'.$d->jamke.'</td><td align="center" width="60">'.$d->kelas.'</td><td>'.$d->mapel.'</td><td>'.$materi.'</td><td>'.$alat_dan_bahan.'</td><td>'.$namaguru.'</td><td></td><td>'.$namasiswa.'</td></tr>';
		$nomor++;
		}
