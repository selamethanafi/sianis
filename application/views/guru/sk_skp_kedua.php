...
<div class="card-body">
<p><a href="<?php echo base_url();?>pkg2/skp" class="btn btn-info">Ke SKP</a></p>
<?php
$tb = $this->db->query("select * from `ppk_pns_kedua` where `kode`='$nippegawai' and `tahun`='$tahun'");
$adatb = $tb->num_rows();
if($adatb == 0)
{
	$this->db->query("insert into `ppk_pns_kedua` (`tahun`,`kode`) values ('$nippegawai', '$tahun')");
}
$tb = $this->db->query("select * from `ppk_pns_kedua` where `kode`='$nippegawai' and `tahun`='$tahun'");
...
echo form_open('pkg2/skskp','class="form-horizontal" role="form"');
