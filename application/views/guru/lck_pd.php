...
atau gunakan Ctrl + W
			<?php
			echo '<table><tr><td colspan="6" align="center"><h3>LAPORAN CAPAIAN KOMPETENSI PESERTA DIDIK</h3></TD><TR><tr><td width="10%">Nama Madrasah</td><td width="2">:</td><td width="25%">'.$this->config->item('sek_nama').'</td><td width="10%">Kelas</td><td width="2">:</td><td>'.$kelas.'</td></tr><tr><td>Alamat</td><td>:</td><td>'.$this->config->item('sek_alamat').'</td><td>Semester</td><td>:</td><td>'.$semester.'</td></tr><tr><td>Nama Peserta Didik</td><td>:</td><td>'.$namasiswa.'</td><td>Tahun Pelajaran</td><td>:</td><td>'.$thnajaran.'</td></tr><tr><td>Nomor Induk / NISN</td><td>:</td><td>'.$nis.' / '.$nisn.'</td><td>Wali kelas</td><td>:</td><td>'.$namawalikelas.'</td></tr></table>';
			...
			$tc =$this->db->query("select * FROM `nilai` WHERE `thnajaran`='$thnajaran' and `semester`='$semester' and `nis` = '$nis' and `status`='Y' order by `kd_mapel`");
			$nomor=1;
			foreach($tc->result() as $c)
			{
				...
				echo '<tr><td width="30" align="center">'.$c->kd_mapel.'</td>';
				echo '<td>'.$c->mapel.'<br />'.$namaguru.'</td>';
				$nilai_pengetahuan = $c->kog;
				$nilai_keterampilan = $c->psi;
				$nilai_afektif = $c->afektif;
				$ket = $c->ket_akhir;
				$kd = $c->kd;
				$kunci = $c->kunci;
				...
				{
					echo '<td align="center" class="table-danger">'.$qa.'</td><td align="center" class="table-danger">'.$qb.'</td>';
				}
				else
				{
				...
				{
					echo '<td align="center" class="table-danger">'.$qc.'</td><td align="center" class="table-danger">'.$qd.'</td>';
				}
				else
				{
				...
				{
					echo '<td align="center" class="table-danger">'.$qe.'</td>';
				}
				if (substr($ket,0,5)=='Belum')
				{
					echo '<td align="center" class="table-danger">'.$ket.'</td>';
				}
				else
				{
					echo '<td align="center" class="table-success">'.$ket.'</td>';
				}

				if ($kunci == 1)
				{
					echo '<td align="center" valign="center"><span class="fa fa-lock"></span>';
				}
				else
				{
					echo '<td align="center" valign="center"><span class="fa fa-lock-open"></span>';
				}
				echo '</td><td align="center">';
				if ($kunci == 1)
				{
					echo '<a href="'.base_url().'guru/detilsiswa/'.$nis.'/'.$id_walikelas.'/'.$item.'/buka/'.$kd.'" class="btn btn-danger" data-confirm="Yakin hendak membuka nilai mapel '.$mapel.'?"><span class="fa fa-lock-open"></span>';
				}
				else
				{
					echo '<a href="'.base_url().'guru/detilsiswa/'.$nis.'/'.$id_walikelas.'/'.$item.'/kunci/'.$kd.'" class="btn btn-warning"  data-confirm="Yakin hendak mengunci nilai mapel '.$mapel.'?"><span class="fa fa-lock"></span></span>';
					$cacahkunci++;
				}
				echo '</td>';
				echo '</tr>';
...
//deskripsi capaian
			...
			echo '<tr><td>Mata Pelajaran</td><td align="center" width="100">Kompetensi</td><td align="center">Deskripsi</td></tr>';
			$th =$this->db->query("select * FROM `nilai` WHERE `thnajaran`='$thnajaran' and `semester`='$semester' and `nis`='$nis' and `status`='Y' order by `mapel`");
			$nomor = 1;
...
//sikap
				...
				echo '<tr><td rowspan="3" width="30%">'.$mapel.'</td><td>Pengetahuan</td><td>'.$desk.'</td></tr><tr><td>Keterampilan</td><td>'.$ketpsi.'</td></tr><tr><td>Sikap Spiritual dan Sosial</td><td>'.$ketafektif.'</td></tr>';
				$nomor++;
			} // akhir deskripsi
			echo '</table>';
			...
			if($cacahkunci == 0)
			{
				//<a href="'.base_url().'pengajaran/rapor/'.substr($thnajaran,0,4).'/'.$semester.'/'.$a->nis.'/'.$status_nilai.'/'.$kurikulum.'" title = "format html  (cocok kalau menggunakan chromium atau google chrome)" target="_blank"><span class="fa fa-print"></span></a>
				?>
				<p class="text-center"><a href="javascript:;" onClick="window.open('<?php echo base_url();?>pdf_report/bukulck/<?php echo $id_thnajaran.'/'.$semester.'/'.$nis.'/akhir';?>','yes','scrollbars=yes,width=550,height=400')" class="btn btn-primary"><span class="fa fa-print"> <strong>LHB PDF</strong> </a>
				<?php
			}
			else
			{
