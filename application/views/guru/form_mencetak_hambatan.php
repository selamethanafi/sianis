...
//============================================================+
// Nama Berkas : form_mencetak_hambatan.php
...
<div class="card">
<div class="card-header"><h3><?php echo 'Mencetak '.$yangdicetak;?></h3></div>
<div class="card-body">

<?php echo form_open('guru/formmencetak/'.$noyangdicetak,'class="form-horizontal" role="form"');?>
<?php 

echo '<input type="hidden" name="yangdicetak" value="Hambatan Belajar Siswa"><input name="kodeguru" type="hidden" value="'.$kodeguru.'">';
?>
<?php
if (!empty($thnajaran))
	{
...
if ((empty($thnajaran)) or (empty($semester)) or (empty($ditandatangani)))
{
	...
	<select name="semester" class="form-control">';
		...
		echo '<option value="2">2</option></select></p></div></div>';
...
else
{
	...
	echo '</select></div></div>';
	echo '<p class="text-center">';
	echo '<input type="hidden" name="diproses" value="oke"><input type="submit" value="Cetak Hambatan Siswa" class="btn btn-primary">&nbsp;&nbsp;&nbsp;<a href="'.base_url().'guru/formmencetak" class="btn btn-info"><b>Batal</b></a></p>';
}
?>
</form>
</div></div></div>
