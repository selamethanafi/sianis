...
if($permanen == 1)
{
	$tb = $this->db->query("SELECT * FROM `skp_skor_guru_kedua` where `id_skp_skor_guru` ='$id' and `nip`='$nip'");
	$adatb = $tb->num_rows();
	if($adatb>0)
	{
		echo form_open('pkg2/realisasiskp/'.$id,'class="form-horizontal" role="form"');
...
$ta=$this->db->query("select * from `skp_skor_guru_kedua` where `tahun`='$tahunsekarang' and `nip`='$nip' order by `nourut`");
...
foreach($ta->result() as $a)
	...
	if(($a->kegiatan == 'Unsur utama') or ($a->kegiatan == 'Unsur Penunjang Tugas Guru') or ($a->kegiatan == 'Unsur PKB'))
		...
		{
			echo '<tr>';
			echo '<td align="center">'.$nourut.'</td><td><a href="'.base_url().'pkg2/realisasiskp/'.$a->id_skp_skor_guru.'" title="Menambah Cacah Realisasi SKP">'.$a->kegiatan.'</a></td><td align="center">'.$a->ak.'</td><td align="center">'.$a->kuantitas.'</td><td align="center">'.$a->kuantitas_r.'</td>';
			$id_skp = $a->id_skp_skor_guru;
			$td = $this->db->query("select * from `skp_realisasi_kedua` where `id_skp`='$id_skp'");
			$adatd = $td->num_rows();
			$bulanrealisasi = '';
			foreach($td->result() as $d)
			{
				...
				{
					$bulanrealisasi .= '<a href="'.base_url().'pkg2/hapusrealisasiskp/'.$d->id_skp_realisasi.'" title="Menghapus Realisasi">'.gantibulan($d->bulan).'</a>';
				}
				else
				{
					$bulanrealisasi .= '<br /><a href="'.base_url().'pkg2/hapusrealisasiskp/'.$d->id_skp_realisasi.'" title="Menghapus Realisasi">'.gantibulan($d->bulan).'</a>';
				}
...
echo '</table>';
	echo '<p class="text-center"><a href="'.base_url().'pkg2/realisasiskp/permanen" class="btn btn-info"><b>Kirim Pemberitahuan kepada Admin</b></a></p> ';
}
else
{
