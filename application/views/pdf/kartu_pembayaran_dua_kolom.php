...
foreach($ta->result() as $da)
{
	...
	if($kolom == 1)
	{
		...
		$this->fpdf->Cell($lebarkartu, $a, '', 1, 0, 'C', 0);
		$kolom = 2;
	}
	else
	{
		$yyy = $ygambarkiri + $y + $yy;
		$this->fpdf->Image('images/logo.jpg',$xgambarkanan,$yyy,30,30);
		$this->fpdf->setXY($kiri+$lebarkartu+10,$ygambar+35);
		$this->fpdf->SetFont('Times','',12);
		$this->fpdf->cell($lebarkartu,5,'KEMENTERIAN AGAMA',0,2,'C',0);
		$this->fpdf->cell($lebarkartu,5,$this->config->item('sek_nama'),0,2,'C',0);
		$this->fpdf->cell($lebarkartu,5,$this->config->item('sek_alamat'),0,2,'C',0);
		...
		if (!empty($foto))
		{	
			$this->fpdf->Image($this->config->item('folderfotosiswa').'/'.$foto.'',$xfotosiswa+$lebarkartu+10,$yfotosiswa,30,40);
		}
		$this->fpdf->setXY($kiri+$lebarkartu+10,$yfotosiswa+43);
		$this->fpdf->cell($lebarkartu,3,'',0,2,'C',0);
		$this->fpdf->SetFont('free3of9','',36);
		$this->fpdf->cell($lebarkartu,5,'*'.$da->nis.'*',0,2,'C',0);
		$this->fpdf->SetFont('Times','',12);
		$this->fpdf->cell(30,3,'',0,2,'C',0);
		$this->fpdf->setX($kiri+$lebarkartu+15);
		$this->fpdf->cell(20,5,'NIS',0,0,'L',0);
		$this->fpdf->cell(60,5,': '.$da->nis,0,2,'L',0);
		$this->fpdf->setX($kiri+$lebarkartu+15);
		$this->fpdf->cell(20,5,'Nama',0,0,'L',0);
		...
		$this->fpdf->SetXY($kiri+$lebarkartu+10,10+$y+$yy);
		$this->fpdf->Cell($lebarkartu, $a, '', 1, 2, 'C', 0);
		$y = $this->fpdf->getY();
		$kolom = 1;
		$kali++;
		$yy= 5;
