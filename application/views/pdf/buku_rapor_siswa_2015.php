...
$this->fpdf->SetTitle("buku rapor permen 53 tahun 2015");
...
//prestasi
	...
	$this->fpdf->MultiCell($lebar,0.5,$catatanwalikelas,1,'L',0);
	$this->fpdf->SetX($x);
	$this->fpdf->Cell(1,1,'',0,2,'L',0);
	...
	if($ywali > 17)
	{
		...
		$this->fpdf->Cell($k3,0.4,$this->config->item('lokasi').', '.date_to_long_string($tanggalcetak),0,2,'L',0);
		$this->fpdf->SetX($x+$k1);
		$yy = $this->fpdf->GetY();
		...
		$this->fpdf->Cell($k3,0.4,$this->config->item('plt').'Kepala '.ucwords($this->config->item('sek_tipe')),0,2,'L',0);
