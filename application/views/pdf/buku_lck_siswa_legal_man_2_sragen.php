...
$this->fpdf->SetTitle("buku lck siswa sebelum permen 53 tahun 2015 versi 2");
...
//	$this->fpdf->Cell(19,0.4,'LAPORAN CAPAIAN KOMPETENSI SISWA',0,2,'C',0);
	...
	foreach($tmapel->result() as $dm)
	{
		...
		else
		{
			...
			{
				$tc = $this->db->query("select * from `nilai` WHERE `thnajaran`='$thnajaran' and `semester`='$semester' and `mapel` like '%prakarya%' and `nis`='$nis' and `status`='Y' limit 0,1");
			...
			//cari guru
			$kodeguru ='??';
			$tf = $this->db->query("select * from `m_mapel` where `mapel`='$mapel' and `thnajaran`='$thnajaran' and `kelas`='$kelas' and `semester`='$semester' limit 0,1");
			$namaguru = '';
...
//			$namaguru .= $mapel;
			...
			{
				$this->fpdf->SetXY($x+$k1+$k2,$yn);
				$this->fpdf->Cell($k3+$k4+$k5+$k6+$k7+$k8+$k9,$selisih,'belum ada nilai atau siswa tidak mengikuti mata pelajaran ini',1,2,'C',0);
			}
				$ymapelatas = $this->fpdf->GetY();	
				if($ymapelatas>22)
				{
	...
	if($semester == '2')
	{
		$tg = $this->db->query("select * from `siswa_prestasi` where `thnajaran`='$thnajaran' and `nis`='$nis' and `valid`='1'");
		if($tg->num_rows()>0)
		{
			$this->fpdf->Cell($lebar,0.5,' ',0,2,'C',0); // spasi kosong
			$this->fpdf->SetX($x);	
			$this->fpdf->Cell(1,0.8,'NO',1,0,'C',0);
			$this->fpdf->Cell(4,0.8,'Prestasi',1,0,'C',0);
			$this->fpdf->Cell($lebar-5,0.8,'Keterangan',1,2,'C',0);
			$this->fpdf->SetX($x);
			$no = 1;
			foreach($tg->result() as $g)
			{
				$this->fpdf->Cell(1,0.6,$no,1,0,'C',0);
				$this->fpdf->Cell(4,0.6,$g->keterangan,1,0,'L',0);
				$this->fpdf->MultiCell($lebar-5,0.6,$g->kegiatan,1,'L',0);
				$this->fpdf->SetX($x);
				$no++;
			}
		}
		$th = $this->db->query("select * from `siswa_organisasi` where `thnajaran`='$thnajaran' and `nis`='$nis' ");
		if($th->num_rows()>0)
		{
			$this->fpdf->Cell($lebar,0.5,' ',0,2,'C',0); // spasi kosong
			$this->fpdf->SetX($x);	
			$this->fpdf->Cell(1,0.8,'NO',1,0,'C',0);
			$this->fpdf->Cell(4,0.8,'Organiasi',1,0,'C',0);
			$this->fpdf->Cell($lebar-5,0.8,'Keterangan',1,2,'C',0);
			$this->fpdf->SetX($x);
			$no = 1;
			...
			{
				$this->fpdf->Cell(1,0.6,$no,1,0,'C',0);
				$this->fpdf->Cell(4,0.6,$h->organisasi,1,0,'L',0);
				$this->fpdf->MultiCell($lebar-5,0.6,$h->keterangan,1,'L',0);
				$this->fpdf->SetX($x);
				$no++;
			}
		}
	...
	$this->fpdf->SetX(12);
	$this->fpdf->Cell(5,0.3,$lokasi.', '.date_to_long_string($tanggalcetak),0,2,'L',0);
	$this->fpdf->SetX($x+1);
	$yy = $this->fpdf->GetY();
...
//	$this->fpdf->Cell(17,0.4,'LAPORAN CAPAIAN KOMPETENSI SISWA',0,2,'C',0);
	...
	foreach($tmapel->result() as $dm)
	{
		...
		else
		{
			...
			//sikap
			$tj = $this->db->query("select * from `afektif` where `mapel`='$mapel' and thnajaran='$thnajaran' and kelas='$kelas' and semester='$semester' and nis='$nis' and `status`='Y'");
			$ketafektif='';
...
//					$this->fpdf->Cell(9,$selisih,'',1,2,'L',0);
	...
	if($ywali > 24)
	{
		...
		$this->fpdf->SetX(11);
		$this->fpdf->Cell(5,0.5,$lokasi.', '.date_to_long_string($tanggalcetak),0,2,'L',0);
		$yy = $this->fpdf->GetY();
		if($ttd == 1)
		{
			$posisix = $posisi_x+ 9;
		...
		$this->fpdf->Cell(0.8,0.5,$plt.'Kepala',0,0,'L',0);
