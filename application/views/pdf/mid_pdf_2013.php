...
if ($ada>0)
{
	...
	$cacah = 0;
		...
		*/
		...
		$this->fpdf->cell($k5,8,'Angka',1,0,'C',0);
		$this->fpdf->cell($k6,8,'Pred',1,0,'C',0);
		$this->fpdf->cell($k7,8,'Ketercapaian',1,2,'C',0);
		...
		foreach($tmapel->result() as $dm)
		{
			...
			if($mapele == 'Peminatan')
			{
				...
				$this->fpdf->MultiCell($k2+$k3+$k4+$k5+$k6+$k7,4,$mapele.' '.$peminatan,0,'L',0);
				$yk7 = $this->fpdf->GetY();
				$sk7 = $yk7 - $yn;
				if($sk7<8)
					{
			...
			{
				$this->fpdf->Cell($k1,4,$komponen,0,0,'C',0);
				$this->fpdf->MultiCell($k2+$k3+$k4+$k5+$k6+$k7,4,$mapele,0,'L',0);
				$yk7 = $this->fpdf->GetY();
				$sk7 = $yk7 - $yn;
				if($sk7<8)
					{
			...
			else
			{
				...
				$this->fpdf->Cell($k5,4,konversi_nilai($nilai_mid),0,0,'C',0);
				$this->fpdf->Cell($k6,4,predikat_nilai($nilai_mid),0,0,'C',0);
				$this->fpdf->MultiCell($k7,4,$ketercapaian,0,'L',0);
				$yk7 = $this->fpdf->GetY();
				$sk2 = $yk2 - $yn;
				$sk7 = $yk7 - $yn;
				$selisih = $sk2;
		...
		$this->fpdf->SetX($x);
		$this->fpdf->cell($k1+$k2+$k3+$k4+$k5+$k6+$k7,6,'Cacah mata pelajaran belum kompeten : '.$cacah.'',1,2,'L',0);//ketercapaian
		$this->fpdf->cell($k1,6,'* : Mata pelajaran pilihan',0,2,'L',0);//ketercapaian		
