...
$this->fpdf->Cell($l,$t,': '.$this->config->item('kode_un_satuan').'-'.substr($thnajaran,7,2).'-'.$this->config->item('kode_un_provinsi').'-'.$this->helper->nomor_un($nis),0,2,'L',0);
$k1 = 2;
$k2 = 8;
$k3 = 2;
$k4 = 6;
...
$pembagi = 0;
$rjr = 0;
$tc = $this->db->query("select * from `m_mapel_ijazah` where `thnajaran`='$thnajaran' and `kelas` = '$kelas' order by no_urut");
if($tc->num_rows()>0)
{
	foreach($tc->result() as $c)
	{
		...
		if(!empty($mapel))
		{
			$te = $this->db->query("select `nis`, `mapel`, `nilai`, `praktik` from `nilai_ujian` where nis='$nis' and mapel='$mapel'");
			$ntulis = 0;
			$npraktik = 0;
			...
			$nujian = ($bobot_ujian_tertulis * $ntulis) + ($bobot_ujian_praktik * $npraktik);

			$td = $this->db->query("select `nis`, `mapel`, `thnajaran`, `semester`, `rata_rapor` from `nilai` where nis='$nis' and mapel='$mapel' and `thnajaran`='$thnajaran' and `semester`='2'");
			if($td->num_rows() == 0)
			{
				$rata_rapor = '?';
			}
		...
		if(empty($komponen))
		{
			if(empty($mapel))
			{
				if(substr($teks_mapel,3,3) == '...')
				{
					$this->fpdf->Cell($k1,$t,$komponen,1,0,'L',0);
					$this->fpdf->Cell($k2,$t,$teks_mapel,1,0,'L',0);
					$this->fpdf->Cell($k3,$t,'',1,0,'C',0);
					$this->fpdf->Cell($k4,$t,'',1,2,'C',0);
				}
				else
				{
					$this->fpdf->Cell($k1+$k2,$t,$teks_mapel,1,0,'L',0);
					$this->fpdf->Cell($k3,$t,'',1,0,'C',0);
					$this->fpdf->Cell($k4,$t,'',1,2,'C',0);
				}
		...
		{
			$this->fpdf->Cell($k1,$t,$komponen,1,0,'C',0);
			$this->fpdf->Cell($k2,$t,$teks_mapel,1,0,'L',0);
			$this->fpdf->Cell($k3,$t,$nilai,1,0,'C',0);
			$this->fpdf->Cell($k4,$t,$dhuruf,1,2,'C',0);
		}
	...
	$rjr = round($rjr,0);
	$this->fpdf->SetX($x1);
	$this->fpdf->Cell($k1+$k2,$t,'Rata rata',1,0,'C',0);
	$this->fpdf->Cell($k3,$t,$rjr,1,0,'C',0);
	if(($rjr>0) and ($rjr<101))
	{
		if(strlen($rjr==1))
		{
			$dhuruf = dengan_huruf($rjr);
		}
		elseif(strlen($rjr==2))
		{
			$dhuruf = dengan_huruf(substr($rjr,0,1)).' '.dengan_huruf(substr($rjr,1,1));
		}
		else
		{
			$dhuruf = dengan_huruf(substr($rjr,0,1)).' '.dengan_huruf(substr($rjr,1,1)).' '.dengan_huruf(substr($rjr,2,1));
		}
	}
	else
	{
