...
class Kapus extends CI_Controller {
	...
	function index()
	{
		...
		$data["judulhalaman"]= 'Perangkat Tugas Tambahan';
		$in=array();
		...
		$query=$this->Guru_model->Tampil_Kapus_Guru($kodeguru,$limit_ti,$offset_ti);
		$tot_hal = $this->Guru_model->Total_Kapus_Guru($thnajaran,$semester,$kodeguru);
      		$config['base_url'] = base_url() . '/index.php/kapus';
		...
		$data["page"]=$page;
		$data['tugase'] = 'kapus';
		$data['namatugas'] = 'Kepala Perpustakaan';
	...
	function proker()
	{
		...
		$data["judulhalaman"]= 'Program Kerja';
			...
			if ($aksi == 'hapus')
				...
				echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/kapus/proker/".$id."'>";
				} 
	...
	function harian()
	{
		$data["nim"]=$this->session->userdata('username');
		$data["nama"]=$this->session->userdata('nama');
		$data["judulhalaman"]= 'Program Harian';
				$aksi=$this->uri->segment(4);
				$data['id_proker'] =$this->uri->segment(5);
				$id='';		
				if ($this->uri->segment(3) === FALSE)
			...
			if ($aksi == 'hapus')
				...
				echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/kapus/harian/".$id."'>";
				} 
	...
	function bpk()
	{
		$data["nim"]=$this->session->userdata('username');
		$data["nama"]=$this->session->userdata('nama');
		$data["judulhalaman"]= 'Pelaksanaan Kegiatan';
				$aksi=$this->uri->segment(4);
				$data['id_proker'] =$this->uri->segment(5);
				$id='';		
				if ($this->uri->segment(3) === FALSE)
			...
			if ($aksi == 'hapus')
				...
				$kodeguru = $this->Guru_model->Idlink_Jadi_Kode_Guru($data["nim"]);
				$this->Guru_model->Hapus_Agenda_Kalab($id_proker,$kodeguru);
				echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/kapus/bpk/".$id."'>";
				} 
	...
	function blpk()
	{
		$data["nim"]=$this->session->userdata('username');
		$data["nama"]=$this->session->userdata('nama');
		$data["judulhalaman"]= 'Laporan Pelaksanaan Kegiatan';
				$aksi=$this->uri->segment(4);
				$data['id_proker'] =$this->uri->segment(5);
				$id='';		
				if ($this->uri->segment(3) === FALSE)
