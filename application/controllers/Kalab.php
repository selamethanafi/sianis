...
class Kalab extends CI_Controller {
	...
	function index()
	{
		...
		$query=$this->Guru_model->Tampil_Kalab_Guru($kodeguru,$limit_ti,$offset_ti);
		$tot_hal = $this->Guru_model->Total_Kalab_Guru($thnajaran,$semester,$kodeguru);
      		$config['base_url'] = base_url() . 'kalab';
		...
		$data["page"]=$page;
		$data['tugase'] = 'kalab';
		$data['namatugas'] = 'Kepala Laboratorium';
	...
	function proker()
	{
		...
		if ($aksi == 'hapus')
		{
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/kalab/proker/".$id."'>";
		}
		$data['id'] = $id;		
		$data['aksi']= $aksi;
		$data['tugase'] = 'kalab';
		...
		{
			$data['judulhalaman'] = 'Program Kerja Kepala Laboratorium';
			$data['tekseditor'] = '';
			$this->load->view('guru/bg_atas',$data);
	...
	function harian()
	{
		...
		if ($aksi == 'hapus')
		{
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/kalab/harian/".$id."'>";
		}
		$data['id'] = $id;		
		$data['aksi']= $aksi;
		$data['tugase'] = 'kalab';
	...
	function bpk()
	{
		...
		if ($aksi == 'hapus')
		{
			...
			$kodeguru = $this->Guru_model->Idlink_Jadi_Kode_Guru($data["nim"]);
			$this->Guru_model->Hapus_Agenda_Kalab($id_proker,$kodeguru);
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/kalab/bpk/".$id."'>";
		}
		$data['id'] = $id;		
		$data['aksi']= $aksi;
		$data['tugase'] = 'kalab';
