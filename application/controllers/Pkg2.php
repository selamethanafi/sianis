...
class Pkg2 extends CI_Controller {
	...
	function skp($id=null)
	{
		...
		$this->load->view('guru/skp_kedua',$data);
	...
	function tambahskp($id=null)
	{
		...
		if ($id=='hapus')
		{
			...
			if(!empty($chat_id_admin_skp))
			{
				...
				$pesanguru = $this->Guru_model->get_Nama($this->session->userdata('username')).' mengajukan pembatalan SKP '.$tahunpenilaian;
			...
			elseif($pakaisms == 1)
			{
				...
				$inpes["TextDecoded"] = $this->Guru_model->get_Nama($this->session->userdata('username')).' mengajukan pembatalan SKP '.$tahunpenilaian.'';
		...
		if ($id=='pkg')
		{
			$this->load->view('guru/skp_pkg_kedua',$data);
		}
		if ($id=='utama')
		{
			$this->load->view('guru/skp_utama_kedua',$data);
		}
/*
		if ($id=='pkb')
		{
			$this->load->view('guru/skp_pkb_kedua',$data);
		}
		if ($id=='tugas')
		{
			$this->load->view('guru/skp_tugas_kedua',$data);
		}
		if ($id=='penunjang')
		{
			$this->load->view('guru/skp_penunjang_kedua',$data);
		}
*/
		if ($id != 'cetakskp')
		{
	...
	function hapusskp($id=null)
	{
		...
		$this->load->model('Guru_model');
		$nip = $this->Guru_model->get_NIP($data["nim"]);
		$this->Guru_model->Hapus_Skp_Kedua($id,$nip);
		redirect('pkg/skp');
	}
	...
	{
		$data["nim"]=$this->session->userdata('username');
		$data["judulhalaman"]= 'Pangkat Golongan Masa Penilaian SKP (KEDUA)';
		$this->load->helper(array('fungsi','pkg'));
		$this->load->model('Guru_model');
...
//			$in['id_pejabat'] = $id_pejabat;
			$in = nopetik($in);
			$this->Guru_model->Update_Parameter_Skp_Kedua($in);
			redirect('pkg2/skp');
		...
		{
			$this->load->view('guru/bg_atas',$data);
			$this->load->view('guru/sk_skp_kedua',$data);
			$this->load->view('shared/bawah');
		}
	}
	...
	function realisasiskp($id=null)
	{
		...
		if($id == 'permanen')
		{
			...
			$chat_id = $this->ref->ambil_nilai('chat_id_admin_skp');
			$token_bot = $this->ref->ambil_nilai('token_bot');
			$pesantelegram = $this->Guru_model->get_Nama($this->session->userdata('username')).' mengirim realisasi SKP kedua '.$tahunpenilaian.'';
			if(!empty($chat_id))
			{
				$this->load->helper('telegram');
				$kirimpesan = kirimtelegram($chat_id,$pesantelegram,$token_bot);
				$this->session->set_flashdata('pesan_info', '<div class="alert alert-info">Realisasi SKP kedua telah disampaikan ke Admin. Mohon bersabar.</div>');
			}
			else
			{
				$inpes["DestinationNumber"]=$this->config->item('nohpadminskp');
				...
				?>
				<script type="text/javascript" language="javascript">
				alert("Realisasi SKP kedua telah disampaikan ke Admin. Terima kasih.");
				</script>
				<?php
			}
			redirect('pkg/skp');
		...
		else
		{
			...
			if((!empty($bulan)) and (!empty($id)))
			{
				...
				$in["nip"] = $nip;
				$in['bulan'] = $bulan;
				$this->Guru_model->Tambah_Realisasi_Skp_Kedua($in);
				redirect('pkg2/realisasiskp');
			}
			$this->load->view('guru/bg_atas',$data);
			$this->load->view('guru/skp_realisasi_kedua',$data);
			$this->load->view('shared/bawah');
		}
	}
	function hapusrealisasiskp()
	{
		$this->load->model('Guru_model');
		$id = $this->uri->segment(3);
		$this->Guru_model->Hapus_Realisasi_SKP_Kedua($id);
		redirect('pkg2/realisasiskp');
	}
	function penilai()
	{
